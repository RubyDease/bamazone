...
//the function to purchase one of the items listed above 
var purchase = function() {
    ...
    prompt.get(productInfo, function(err, res) {
        ...
        //the variable entered above is pushed to the productPurchased array 
        productPurchased.push(custPurchase);

        //connects to the mysql database and selects the item the user selected based on the item ID
        connection.query('SELECT * FROM Products WHERE ItemID=?', productPurchased[0].itemID, function(err, res) {
            ...
            //if the stock quantity available is less than the qyantity that the user wanted to purchase then the user will receive notification that product is out of stock 
            if (res[0].StockQuantity < productPurchased[0].Quantity) {
                console.log('That product is out of stock!');
                connection.end();

                //otherwise if the stock amount available is more than or equal to the amount being asked for then the purchase is continued 
